...
type ViewResponseMetadata struct {
	...
	// Retrieval fields are populated from the semantic-retrieval gate so
	// the context pane can show why a file was admitted or rejected.
	DecisionAllowed  bool    `json:"decision_allowed,omitempty"`
	DecisionReason   string  `json:"decision_reason,omitempty"`
	DecisionScore    float64 `json:"decision_score,omitempty"`
	DecisionMaxLines int     `json:"decision_max_lines,omitempty"`
}
...
- Default reading limit is 200 lines
- Lines longer than 2000 characters are truncated
- Cannot display binary files or images
- Images can be identified but not displayed

TIPS:
- For code exploration, rely on semantic codebase-memory ranking before loading files
- Read only specific, high-value sections instead of entire source files
...
func (v *viewTool) Run(ctx context.Context, call ToolCall) (ToolResponse, error) {
	...
	var decision llmcontext.Decision
	if state := llmcontext.StateFromContext(ctx); state != nil {
		decision = state.CheckFile(ctx, filePath)
		if !decision.Allowed {
			return NewTextErrorResponse(fmt.Sprintf("Semantic retrieval rejected this read: %s. Query codebase memory or read one of the PageRank-selected files before loading lower-value files.", decision.Reason)), nil
		}
		if decision.MaxLines > 0 && params.Limit > decision.MaxLines {
			params.Limit = decision.MaxLines
		}
	}
	...
	return WithResponseMetadata(
		...
		ViewResponseMetadata{
			...
			DecisionMaxLines: decision.MaxLines,
		},
	), nil
}
